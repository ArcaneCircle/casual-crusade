...
export class Level {
  ...
  public next(): void {
    ...
    if (this.starter) {
      ...
      this.board[2].content.setPosition(
        this.offset.x + TILE_WIDTH,
        this.offset.y + TILE_HEIGHT,
      );
    }

    const center = this.board[2].p;
    const tiles = this.getPossibleRewardSpots()
      .sort(randomSorter)
      .sort((a, b) => {
        return distance(a.p, center) - distance(b.p, center);
      });
    tiles
      .slice(0, Math.max(0, this.level - Math.max(0, this.level - 8) * 3))
      .forEach((tile) => {
  ...
  private isInRange(
    t: Tile /*, minX: number, maxX: number, minY: number, maxY: number*/,
  ): boolean {
  ...
  private getFromAllEdgeTile(/*minX: number, maxX: number, minY: number, maxY: number*/): Vector {
    const tiles = this.board
      .filter((tile) => tile.getNeighbours(this.board).length < 4)
      .filter((t) => this.isInRange(t /*, minX, maxX, minY, maxY*/));
    const spots: Vector[] = [];
    tiles.forEach((tile) => {
      spots.push(
  ...
  private getPossibleRewardSpots(): Tile[] {
    return [
      ...this.board.filter(
        (tile) =>
          tile != this.board[2] && tile.getNeighbours(this.board).length < 4,
      ),
    ];
  }
  ...
  private edgeOrZero(tile: Tile, dir: Vector): Vector {
    return !this.board.some(
      (t) =>
        t.index.x == tile.index.x + dir.x && t.index.y == tile.index.y + dir.y,
    )
      ? {
