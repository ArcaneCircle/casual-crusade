...
export class Dude extends Entity {
  ...
  public findPath(to: Tile, game: Game, level: Level): void {
          ...
          if (index > 0) {
            ...
            if (game.remoteMulti) {
              const neighbours = tile
                .getNeighbours(level.board)
                .filter((t) => t.content && t.content.is("o"));
    ...
    setTimeout(
      () => game.checkLevelEnd(),
      this.path.length * moveDuration * 1000 + 600,
    );
  }
  ...
  private findNext(
    ...
    const steps = from.content
      .getConnections()
      .filter(
        (tile) =>
          !visited.includes(tile) ||
          (free.some((f) => tile.content.is(f)) &&
            visited.filter((t) => t == tile).length < 5),
      );
    if (from == to) {
