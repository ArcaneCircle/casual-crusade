...
export class Game extends Entity {
  ...
  private splash = new Container(WIDTH * 0.5, 140, [
    new TextEntity("", 40, WIDTH * 0.5, 120, -1, ZERO, { shadow: 4 }),
  ]);
  ...
  public getWilds(gem: GemColor): GemColor[] {
    return [
      ...this.wilds.filter((w) => w.first == gem).map((w) => w.second),
      ...this.wilds.filter((w) => w.second == gem).map((w) => w.first),
    ];
  }
  ...
  public nextLevel(): void {
    ...
    const hits = this.level.board.filter(
      (tile) => !tile.content && !tile.reward,
    );
    const delay = 200;
    ...
    [...hits].sort(randomSorter).forEach((hit, i) => {
      ...
      setTimeout(
        () => {
          ...
          this.effects.add(
            new LineParticle(sky, p, 0.4, 10, "#ffffcc99", random(10, 20)),
          );
          this.life--;
          hit.hidden = true;
          this.audio.boom();
    ...
    setTimeout(
      () => {
        ...
        this.blinders.close(() => {
          ...
          const sortedX = [...this.level.board]
            .filter((t) => !t.reward)
            .map((t) => t.index.x)
            .sort((a, b) => a - b);
          const sortedY = [...this.level.board]
            .filter((t) => !t.reward)
            .map((t) => t.index.y)
            .sort((a, b) => a - b);
          const x =
            -(sortedX[0] - 1 + sortedX[sortedX.length - 1] - 1) *
            0.5 *
            TILE_WIDTH;
          const y =
            -(sortedY[0] - 1 + sortedY[sortedY.length - 1] - 1) *
            0.5 *
            TILE_HEIGHT;
  ...
  public checkLevelEnd(): void {
    ...
    if (
      handCards.length == 0 ||
      this.level.isFull() ||
      !handCards.some((c) => c.getPossibleSpots().length > 0)
    ) {
      this.nextLevel();
    }
  }
  ...
  public add(card: CardData, shuffles = true, permanent = false): void {
    ...
    this.deck.push(card);
    if (shuffles)
      this.deck = [...this.deck].sort(() => (Math.random() < 0.5 ? 1 : -1));
    this.reposition();
  }
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    ...
    // this.helps.draw(ctx);
    [...this.cards, this.dude, this.pile]
      .sort(sortByDepth)
    ...
    for (let i = 0; i < 20; i++) {
      ...
      this.effects.add(
        new RectParticle(
          ...
          size,
          0.2 + Math.random() * 0.5,
          { x: -3 + Math.random() * 6, y: -7 * Math.random() },
          opts,
        ),
      );
    }
  ...
  public loot(tile: Tile): void {
    const chests = tile
      .getChests(this.level.board)
      .filter((n) => n.reward && !n.looted);
    if (chests.length > 0) {
      setTimeout(() => {
        this.audio.frog();
        this.audio.open();
        chests.forEach((c) => {
          ...
          this.effects.add(
            new LineParticle(p, offset(sky, 0, -10), duration, 10, "#ffffff55"),
          );
          this.effects.add(
            new LineParticle(
              offset(p, -5, 0),
  ...
  public reposition(): void {
    const handCards = [...this.cards.filter((c) => !c.isLocked())].sort(
      (a, b) => a.p.x - b.p.x,
    );
    this.pile.count = this.deck.length;
    handCards.forEach((c, i) =>
      c.move(
        offset(this.p, (i - handCards.length * 0.5 + 0.5) * TILE_WIDTH, 0),
        0.15,
      ),
    );
    this.pile.move(
      offset(this.p, -(handCards.length * 0.5 + 1) * TILE_WIDTH, 0),
      0.15,
    );
  }
